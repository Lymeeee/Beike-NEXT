    ...
    switch (status) {
      case CourseSelectionStatus.idle:
        return TaskStatusInfo(
          color: (c) => Theme.of(c).colorScheme.onSurfaceVariant,
          icon: Icons.timer,
          text: '待提交',
        );
      ...
      case CourseSelectionStatus.errorTimeout:
        return TaskStatusInfo(
          color: (c) => Theme.of(c).colorScheme.secondary,
          icon: Icons.timer_off,
          text: '超时',
        );
      case CourseSelectionStatus.errorApi:
        return TaskStatusInfo(
          color: (c) => Theme.of(c).colorScheme.error,
          icon: Icons.error,
          text: '失败',
        );
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Divider(color: Theme.of(context).colorScheme.outlineVariant.withValues(alpha: 0.2)),
          const SizedBox(height: 12),
          Row(
            children: [
              Icon(
          ...
          Row(
            children: [
              ...
              Text(
                statusInfo.text,
                style: TextStyle(
                  color: statusInfo.color(context),
                  fontWeight: FontWeight.w500,
                  fontSize: 14,
                ),
              ),
      ...
      return AnimatedBuilder(
        ...
        builder: (context, child) {
          return Opacity(
            ...
            child: Icon(statusInfo.icon, color: statusInfo.color(context), size: 20),
          );
        },
      );
    } else {
      return Icon(statusInfo.icon, color: statusInfo.color(context), size: 20);
    }
  }
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('成功：$successCourseCount / $totalCourseCount 门课程'),
            const SizedBox(height: 8),
            Text('已停止后续提交。', style: TextStyle(color: Theme.of(context).colorScheme.secondary)),
          ],
        ),
        actions: [
          FilledButton(
